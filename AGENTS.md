...
- **Web validation:** Do not run `npm run build` in `apps/web` unless the user explicitly asks for a production build. It overwrites the user's running dev build and forces them to restart the app. Prefer `npm run typecheck` for routine web validation.
- **Docs after larger changes:** After any larger product, behavior, API, route, or workflow change, update the relevant README/docs and `CHANGELOG.md` in the same turn. At minimum check root `README.md`; also update app/plugin-specific READMEs when the changed surface has one.
